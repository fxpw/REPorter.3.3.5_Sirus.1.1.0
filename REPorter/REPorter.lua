...
-- REPorter:SetScript("OnEvent", OnEvent);
...
RE.MapSettings = {
	-- HE -- Height ;  HO - HorizontalScroll ; VE - VerticalScroll
	...
	["TempleCityBG"]         = {["HE"] = 400, ["HO"] = 160, ["VE"] = 45, ["WI"] = 500,  ["pointsToWin"] = 1500, ["WorldStateNum"] = 1, ["StartTimer"] = 240},
};
		...
		for i=1, RE.numVehicles do
			...
			if vehicleX and not isPlayer and vehicleType ~= "Idle" then
				...
				if RE.CurrentMap == "STVDiamondMineBG" then
					RE.BGVehicles[i].texture:SetTexture(_G["WordldStateTopCenterFrameLeftBarContainerPOIButton"..i.."Texture"]);
				else
...
function REPorter_Update(NotResetHealth) -- FH 1
	REPorterExternal:SetScript("OnUpdate", nil);
...
function REPorter_NodeChange(newTexture, nodeName) -- FH 3
	RE.ShefkiTimer:CancelTimer(RE.POINodes[nodeName]["timer"], true);
	RE.POINodes[nodeName]["timer"] = nil;
	Debug("Node change. %s, %s", nodeName, newTexture)
